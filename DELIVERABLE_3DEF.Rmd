...
# Model na?ve
#m0<-glm(Audi ~ 1, family="binomial", data=df_test)#si no poses data=df_test dona error in length
#prob.audi <- m0$fit
prob.audi <- predict(m0, newdata=df_test, type="response")#com abans
audi.est <- ifelse(prob.audi<0.5,0,1)
tt<-table(audi.est,df_test$Audi);tt
100*tt[1,1]/sum(tt)
```

Finally, we execute the predictions and generate a confusion matrix with the results of it. To conclude, the diagonal of this confusion matrix shows a performance of 78.4% hit rate in our prediction model against our testing data set, and the model m0 without any coefficient would give us a 79.1% of hit rate. This means that the regressors are not adequate to predict if a car is Audi or not.
Curve ROC of our chosen method is displayed above.
Also, we can say that the model has a tendency to predict not Audi.(CREC Q VA DIR ALGO AIXI EL PROFE)
