...
### years_sell #PROFE AVUI HA DIT Q MILLOR NO FER OUTLIERS DE YEARS_SELL PRO NO RECORDO BEN BE PQ
...
### Correlation 
```{r}
#Nomes amb variables numeriques
#Com mes correlacio més tendencia a tenir valor anomals <- justificació 
```
...
round(quantile(res.impca$completeObs[,"years_sell"],seq(0,1,0.1),na.rm=T),dig=1)
```
...
df$aux_tax<-factor(cut(df$tax,breaks=c(0, 125, 145, 570),include.lowest = T ))
summary(df$aux) 
...
As we can see in the output condes$quali, model variable has a high correlation, i.e. certain models have high and/or low prices.

Regarding the output of condes$category, we can feature cars with engineSize 5.2 which on average cost 57485.70948£ more than baseline price/average price and cars with model Audi-R8 which on average cost 56033.34832£ than baseline price. we can feature cars with model Audi-Q8 which on average cost 43576.8483£ more than baseline price/average price 
