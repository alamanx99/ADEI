...
plot(m7)
```
He have 30 outliers on the regression. 
...
//ALTERNATIVA 2
```{r}
m3 <- update(m1, ~.+fuelType*transmission+fuelType*mpg+transmission+engineSize_num,data=df_train[!df_train$mout=="YesMOut",])
anova(m2,m3,test="Chisq")
summary(m3)
```
As we can see the decrease in deviance is significant, because the p-value of anova test is less than 0.05, so we can say that Audi is related to fuelType and mpg.
...
We can see: 
Within/in the petrol category, the mpg variable causes the probability of being Audi decrease by (exp(-0.057-1.98+0.022) = 0.13 -> 100*(1 - 0.13) = 87 %) 87 %.

The fact that a car is petrol and with semiAuto transmission makes the probability of being Audi decrease by (exp(-1.98-0.82+1) = 0.5 -> 100*(1-0.5) = 50 %) decrease by 50 %.
The fact that a car is petrol and with automatic transmission makes the probability of being Audi decrease by (exp(-1.98-0.45+0.66) 48 %.
...
```
He have 3 outliers on the regression. 
...
```
He have 2 outliers on the regression. 
