...
We need to square the tax variable, take the square root of the mileage variable and 

```{r}
df2 <- df[!df$mout=="YesMOut",]

m3<-lm(log(price)~sqrt(mileage)+poly(tax,2)+ I(mpg^(-3/2))+years_sell,data=df2)
summary(m3)
vif(m3) 
par(mfrow=c(2,2))
plot(m3, id.n=0)
