...
We can see that the only significant interaction is that within/in the petrol category, the mpg variable causes the logarithm of the price decrease by 0.0068 units, because the p-value is less than 0.05. 
...
m2 <- glm(Audi~ poly(mileage,2)+poly(tax,2)+poly(mpg,2)+poly(years_sell,2),family="binomial",data=df_train[!df_train$mout=="YesMOut",])
summary(m2)
marginalModelPlots(m2)
```
Trying the same transformations that in the previous model (price as target), except the mpg transformation, we can see that now the useful variables are mpg, years_sell and tax. However, in the marginalModelPlots we can see that this model captures the data worse (bottom left plot). 

Trying also the quadratic terms of the variables we can see that the only useful variable is mpg. Also, in the marginalModelPlots we can see that this model captures the data worse (bottom left plot).  

So we have decided not to apply transformations on the regressors. 
...
We can see that the only significant interactions is:
The fact that a car is diesel and with manual transmission makes the probability of being Audi increases by (exp(0.66) = 1.93 -> 100*(1.93-1) = 93 %) by 93 %, because the p-value is less than 0.05. 
...
We can see that the only significant interactions are:
The fact that a car is petrol and with semiAuto transmission makes the probability of being Audi increases by (exp(0.81) = 2.25 -> 100*(2.25-1) = 125 %) 125 %, because the p-value is less than 0.05. 

The fact that a car is petrol and with automatic transmission makes the probability of being Audi increases by (exp(0.47) = 1.6 -> 100*(1.6-1) = 60 %) 60 %, because the p-value is less than 0.05.
### Factor and covariate interaction
//ALTERNATIVA 1
```{r}
...
The fact that a car is diesel and with manual transmission makes the probability of being Audi increases by (exp(1.77) = 5.87 -> 100*(5.87 - 1) = 487) 487 %, because the p-value is less than 0.05. 

The fact that a car is diesel and with automatic transmission makes the probability of being Audi increases by (exp(1.32) = 3.74 -> 100*(3.74-1) = 274.3 %) 274.3 %, because the p-value is less than 0.05. 

That within/in the petrol category, the mpg variable causes the logarithm of the price increases by (exp(0.022) = 1.02 -> 100*(1.02-1) = 2 %) 2 %, because the p-value is less than 0.05. 
...
We can see that the only significant interactions are:
The fact that a car is petrol and with semiAuto transmission makes the probability of being Audi increases by (exp(1) = 2.72 -> 100*(2.72 - 1) = 172 %) 172 %, because the p-value is less than 0.05. 
The fact that a car is petrol and with automatic transmission makes the probability of being Audi increases by (exp(0.66) = 1.93 -> 100*(1.93-1) = 93 %) 93 %, because the p-value is less than 0.05. 
That within/in the petrol category, the mpg variable causes the logarithm of the price increases by (exp(0.022) = 1.02 -> 100*(1.02-1) = 2) 2 %, because the p-value is less than 0.05. 
...
Curve ROC of our chosen method is displayed at the end.(EXPLICACIO D'UN DELIVERABLE D'INTERNET POSAR ELS NOSTRES PERCENTATGES I JA)
