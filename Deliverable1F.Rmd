...
Boxplot(df$tax)
boxplot(df$tax)
df$f.tax<-factor(cut(df$tax/1000,breaks=c(quantile(df$tax,seq(0,1,0.25),na.rm=TRUE)/1000),include.lowest = T ))
summary(df$f.tax)
...
Boxplot(df$mpg)
boxplot(df$mpg)
df$f.mpg<-factor(cut(df$mpg/1000,breaks=c(quantile(df$mpg,seq(0,1,0.25),na.rm=TRUE)/1000),include.lowest = T ))
summary(df$f.mpg)
...
boxplot(df$years_sell)
df$f.years_sell<-factor(cut(df$years_sell/1000,breaks=c(quantile(df$years_sell,seq(0,1,0.25),na.rm=TRUE)/1000),include.lowest = T ))
