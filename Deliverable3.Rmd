...
plot(m7)
```
He have 30 outliers on the regression. As we can see in the Boxplot the 4853 and 1725 observations are the most significant ones. 
We have 63 a priori influential observations.
We have 122 a posteriori influential observations.
...
plot(m7)
```
He have 30 outliers on the regression. As we can see in the Boxplot the 1725 and 4853 observations are the most significant ones. 
We have 97 a priori influential observations.
We have 161 a posteriori influential observations.
...
We can say that against more mpg less probability of being Audi, against more years_sell more probability of being Audi, that hybrid and petrol cars have less probability of being Audi and semiAuto cars have less probability of being Audi. 

More specifically:
Increasing by 1 unit mpg then exp(-0.05*1) = 0.95 -> 100*(1-0.95) = 5 %, the probability of being Audi decreases by 5 %. 
Increasing by 1 unit years_sell the probability of being Audi increases by 12.3 %. 
Being hybrid then exp(-1.66) = 0.19 -> 100*(1-0.19) = 81 %, the probability of being Audi decreases by 81 %. 
Being petrol the probability of being Audi decreases by 30 %.
Being semi-automatic the probability of being Audi decreases by 27 %.
...
summary(m3)
```
As we can see the decrease in deviance is significant, because the p-value of anova test is less than 0.05, so we can say that Audi is not related to fuelType and transmission.
...
summary(m3)
```
As we can see the decrease in deviance is not significant, because the p-value of anova test is greater than 0.05, so we can say that Audi is not related to fuelType and transmission.

### Factor and covariate interaction
//ALTERNATIVA 1
```{r}
m3 <- update(m1, ~.+fuelType:mpg+transmission+engineSize_num,data=df_train[!df_train$mout=="YesMOut",])
anova(m2,m3,test="LR")
summary(m3)
```
As we can see the decrease in deviance is not significant, because the p-value of anova test is greater than 0.05, so we can say that Audi is not related to fuelType and mpg.
...
summary(m3)
```
Our best model (model that has the minimum possible deviance with/using the least number of possible variables) has a deviance of . 
...
Boxplot(abs(rstudent(m3)))
llres <- which(abs(rstudent(m3))>2.3);llres
Boxplot(hatvalues(m3),id=list(labels=row.names(dfwork)))
influencePlot(m3, id=list(n=10))
Boxplot(cooks.distance(m3),id=list(labels=row.names(dfwork)))
llout<-which(abs(cooks.distance(m3))>0.02);length(llout)
llrem<-unique(c(llout,llres));llrem

m4<-glm(Audi ~ mpg + years_sell + fuelType + transmission + 
    engineSize_num, family="binomial",data=dfwork[-llrem,])
summary(m4)
