...
But/However, we can see that residuals are neither homoscedastic nor normal. So, we have to apply transformations. 
...
As we can see the transformation needed is a logarithmic transformation. 
...
Also, we can see that residuals are now homoscedastic and normal. In addition, we can see some influential observation. 
...
m4 <- update(m3, ~.+model+year+engineSize,data=df[!df$mout=="YesMOut",])
