...
m3<-lm(log(price)~sqrt(mileage)+poly(tax,3)+ I(mpg^(-1/2))+years_sell,data=df2)#DECIDIR, I(mpg^(-3/2))
m3<-lm(log(price)~sqrt(mileage)+poly(tax,3)+ I(mpg^(-1/3))+years_sell,data=df2)
...
df[which(df$engineSize_num<=2),"engineSize_sum"] <- "small_engine"
df[which((df$engineSize_num>2) & (df$engineSize_num<=3)),"engineSize_sum"] <- "medium_engine"
df[which(df$engineSize_num>3),"engineSize_sum"] <- "large_engine"
df$engineSize_num<-factor(df$engineSize_num)
